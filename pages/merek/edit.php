...
$title = 'Edit Merek Motor';
$id = (int)($_GET['id'] ?? 0);
$data = $conn->query("SELECT * FROM merek WHERE id = $id")->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        ...
        $stmt = $conn->prepare("UPDATE merek SET nama = ? WHERE id = ?");
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <h2 class="text-xl font-bold text-white">Edit Merek Motor</h2>
    </header>
