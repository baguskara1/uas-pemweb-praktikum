...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_pelanggan) || empty($id_merek) || empty($plat_no)) {
        ...
        $stmt = $conn->prepare("INSERT INTO kendaraan (id_pelanggan, id_merek, plat_no, model, cc, tahun) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('iissii', $id_pelanggan, $id_merek, $plat_no, $model, $cc, $tahun);
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">Tambah Kendaraan</h2>
                ...
                <form method="POST">
                    <div class="space-y-6">
                        <div>
                            ...
                            <select id="id_pelanggan" name="id_pelanggan"
                                ...
                                <?php while ($p = $pelanggan->fetch_assoc()): ?>
                                    <option value="<?= $p['id'] ?>" class="bg-[#161622]" <?= (($_POST['id_pelanggan'] ?? '') == $p['id']) ? 'selected' : '' ?>>
                        ...
                        <div>
                            ...
                            <select id="id_merek" name="id_merek"
                                ...
                                <?php while ($m = $merek->fetch_assoc()): ?>
                                    <option value="<?= $m['id'] ?>" class="bg-[#161622]" <?= (($_POST['id_merek'] ?? '') == $m['id']) ? 'selected' : '' ?>>
                        ...
                        <div>
                            ...
                            <input type="text" id="plat_no" name="plat_no" value="<?= htmlspecialchars($_POST['plat_no'] ?? '') ?>"
                                class="w-full px-4 py-3 bg-[#0a0a0f] border border-[#2a2a3a] rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[#e60000]"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="cc" name="cc" value="<?= htmlspecialchars($_POST['cc'] ?? '') ?>"
                                    class="w-full px-4 py-3 bg-[#0a0a0f] border border-[#2a2a3a] rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[#e60000]"
                            ...
                            <div>
                                ...
                                <select id="tahun" name="tahun"
                                    ...
                                    <?php for ($th = date('Y'); $th >= 2000; $th--): ?>
                                        <option value="<?= $th ?>" class="bg-[#161622]" <?= (($_POST['tahun'] ?? date('Y')) == $th) ? 'selected' : '' ?>>
                                            <?= $th ?>
                                        </option>
                                    <?php endfor; ?>
                                </select>
