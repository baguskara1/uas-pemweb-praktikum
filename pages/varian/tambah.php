...
$title = 'Tambah Varian Jasa';

// Ambil data jasa dan merek untuk dropdown
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_jasa) || empty($id_merek)) {
        ...
        $stmt = $conn->prepare("INSERT INTO varian_jasa (id_jasa, id_merek, cc_min, cc_max, nama_varian, total_harga, is_custom) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiiisii", $id_jasa, $id_merek, $cc_min, $cc_max, $nama_varian, $total_harga, $is_custom);

        if ($stmt->execute()) {
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Varian jasa berhasil ditambahkan.'];
            header('Location: index.php');
            exit;
        } else {
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Gagal menambahkan varian: ' . $conn->error];
        }
        $stmt->close();
    }
}
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">Tambah Varian Jasa</h2>
