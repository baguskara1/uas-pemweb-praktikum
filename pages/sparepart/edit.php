...
$sparepart = $conn->query("SELECT * FROM sparepart WHERE id = $id");
if ($sparepart->num_rows === 0) {
    $_SESSION['flash'] = ['type' => 'error', 'message' => 'Sparepart tidak ditemukan.'];
    header('Location: index.php');
    exit;
}
$data = $sparepart->fetch_assoc();

// Ambil data kategori
$kategori = $conn->query("SELECT * FROM kategori_sparepart ORDER BY nama ASC");

function handle_upload($id, $old_file = null)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($id_kategori)) {
        ...
        $gambar = $data['gambar']; // keep old by default
        $upload_result = handle_upload($id, $data['gambar']);
        if ($upload_result === false) {
            // Upload failed — use old gambar, just show warning
            $warn = 'Foto gagal diperbarui, menggunakan foto lama.';
        } elseif ($upload_result !== null) {
            $gambar = $upload_result;
        }

        $stmt = $conn->prepare("UPDATE sparepart SET id_kategori = ?, kode = ?, nama = ?, stok = ?, harga_beli = ?, harga_jual = ?, gambar = ? WHERE id = ?");
        $stmt->bind_param("issiiisi", $id_kategori, $kode, $nama, $stok, $harga_beli, $harga_jual, $gambar, $id);
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['flash'] = ['type' => 'error', 'message' => 'Gagal memperbarui sparepart: ' . $conn->error];
        }
        $stmt->close();
    }
}
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">Edit Sparepart</h2>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="id_kategori" name="id_kategori" required
                                ...
                                <?php while ($kat = $kategori->fetch_assoc()): ?>
                                    <option value="<?= $kat['id'] ?>" <?= $data['id_kategori'] == $kat['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="mb-3">
                                <img src="../../<?= htmlspecialchars($data['gambar']) ?>"
                                    class="w-24 h-24 rounded-xl object-cover border border-[#2a2a3a]"
                                    alt="Foto sparepart saat ini">
                                <p class="text-gray-500 text-xs mt-1">Foto saat ini</p>
                            </div>
                            <?php endif; ?>
                            ...
                            <p class="text-gray-500 text-xs mt-1">Format: JPG, PNG, WebP. Maks 2MB. Biarkan kosong jika tidak ingin mengganti foto.</p>
                        </div>
                    </div>
