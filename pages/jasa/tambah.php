...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_kategori <= 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO jasa (id_kategori, nama, deskripsi) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $id_kategori, $nama, $deskripsi);
        if ($stmt->execute()) {
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">Tambah Jasa</h2>
        </div>
    </header>
