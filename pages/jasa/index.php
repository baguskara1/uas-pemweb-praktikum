...
if ($search) {
    ...
    $where = "WHERE j.nama LIKE '%$search_safe%' OR k.nama LIKE '%$search_safe%'";
}

$total = $conn->query("SELECT COUNT(*) FROM jasa j JOIN kategori_jasa k ON j.id_kategori = k.id $where")->fetch_row()[0];
$total_pages = ceil($total / $limit);

$query = "SELECT j.*, k.nama AS nama_kategori, k.icon 
          FROM jasa j 
...
<div class="flex-1 flex flex-col w-full min-w-0">
    <header class="bg-[#0d0d1a] border-b border-[#2a2a3a] px-4 md:px-6 py-4 pl-16 md:pl-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">Data Jasa</h2>
        </div>
    </header>
                    ...
                    <table class="table-custom w-full">
                        <thead>
                            <tr class="bg-[#0a0a0f]">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Nama Jasa</th>
                        ...
                        <tbody>
                            <?php $no = $offset + 1; while ($row = $result->fetch_assoc()): ?>
                                <tr class="border-t border-[#2a2a3a] hover:bg-white/[0.02]">
                                    ...
                                    <td class="px-4 py-3 text-sm font-medium text-white"><?= htmlspecialchars($row['nama']) ?></td>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-400 max-w-xs truncate"><?= htmlspecialchars($row['deskripsi'] ?? '-') ?></td>
                                    <td class="px-4 py-3 text-sm">
                                        ...
                                        <a href="hapus.php?id=<?= $row['id'] ?>" 
                                           ...
                                           title="Hapus"
                                           onclick="return confirm('Yakin ingin menghapus jasa &quot;<?= htmlspecialchars($row['nama']) ?>&quot;?')">
